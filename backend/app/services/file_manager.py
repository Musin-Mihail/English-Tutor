...
class FileManager:
    def __init__(self, data_dir: str = "app/data"):
        ...
        self.table_template = os.path.join(
            self.templates_dir, "ENGLISH_performance_table.template.md"
        )
        self.journal_template = os.path.join(
            self.templates_dir, "ENGLISH_training_journal.template.md"
        )
        self._init_files()

    def _init_files(self):
        """Создает рабочие файлы из шаблонов, если они отсутствуют."""
        if not os.path.exists(self.data_dir):
            os.makedirs(self.data_dir)
        if not os.path.exists(self.table_path):
            if os.path.exists(self.table_template):
                print(f"Creating {self.table_path} from template...")
                shutil.copy(self.table_template, self.table_path)
            else:
                print(f"Warning: Template not found at {self.table_template}")
        if not os.path.exists(self.journal_path):
            if os.path.exists(self.journal_template):
                print(f"Creating {self.journal_path} from template...")
                shutil.copy(self.journal_template, self.journal_path)
            else:
                print(f"Warning: Template not found at {self.journal_template}")
